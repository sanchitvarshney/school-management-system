  ...
  return (
    <div className="space-y-3">
      <p className="text-xs text-gray-500">
        P = Present · A = Absent · L = Late · S = Sunday (auto)
      </p>
      <div className="overflow-x-auto rounded-xl border border-gray-200">
        <table className="w-full text-sm border-collapse min-w-[600px] bg-white">
          <thead>
            <tr className="border-b border-gray-200 bg-gray-50 text-gray-600">
              <Th className="py-2 pr-4 text-left w-12 sticky left-0 bg-gray-50 border-r border-gray-200 z-10">
                MONTH
              </Th>
              {Array.from({ length: 31 }, (_, i) => i + 1).map((day) => (
                <Th
                  key={day}
                  className="py-1.5 px-0.5 text-center min-w-[24px] w-8 font-normal border-r border-gray-100 last:border-r-0"
                >
                  {day}
                </Th>
              ))}
          ...
          <tbody className="text-gray-800">
            {months.map(({ year, month, label }) => {
              ...
              return (
                <tr
                  key={`${year}-${month}`}
                  className="border-b border-gray-100 last:border-b-0 group hover:bg-indigo-50/80 transition-colors"
                >
                  <Td className="py-1.5 pr-4 font-medium sticky left-0 border-r border-gray-200 bg-gray-50/90 group-hover:bg-indigo-50/80 z-10 transition-colors">
                    {label}
                  </Td>
                  {Array.from({ length: 31 }, (_, i) => {
                    ...
                    if (day > days)
                      return (
                        <Td
                          key={day}
                          className="py-0.5 px-0.5 text-center min-w-[24px] w-8 border-r border-gray-100 last:border-r-0"
                        />
                      );
                    ...
                    return (
                      <Td
                        key={day}
                        className={`py-0.5 px-0.5 text-center min-w-[24px] w-8 border-r border-gray-100 last:border-r-0 ${color} transition-colors`}
                      >
                        {status}
                      </Td>
                    );
...
export function StudentProfileView({
  ...
  >([
    ...
    {
      id: "doc-2",
      name: "Transfer Certificate (Last School)",
      type: "PDF",
      date: "15 Jan 2024",
      size: "180 KB",
    },
  ...
  const [examExpandedId, setExamExpandedId] = useState<string | null>(
    "mid-term",
  );
  ...
  >([
    {
      id: "note-1",
      date: "15 Mar 2025",
      text: "Parent meeting scheduled for next week. Guardian requested callback regarding transport.",
    },
    {
      id: "note-2",
      date: "10 Mar 2025",
      text: "Good performance in science project. Recommended for school exhibition.",
    },
    {
      id: "note-3",
      date: "05 Mar 2025",
      text: "Submitted leave application for 12–14 Mar (family function). Approved.",
    },
  ]);
        ...
        <div className="space-y-4 text-sm">
          ...
          <SectionFieldset title="Admission / Class">
            <div className="space-y-0">
              ...
              <InfoRow
                label="Class"
                value={classNameById.get(student.classId)}
              />
              <InfoRow
                label="Section"
                value={sectionNameById.get(student.sectionId)}
              />
            </div>
          </SectionFieldset>
          ...
          <SectionFieldset title="Documents">
            <ul className="space-y-3 text-sm">
              {documents.map((doc) => (
                <li
                  key={doc.id}
                  className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-gray-200 bg-gray-50/50 px-4 py-3"
                >
                  <div className="flex items-center gap-3">
                    <span className="rounded-lg bg-indigo-100 px-2 py-1 text-xs font-medium text-indigo-700">
                      {doc.type}
                    </span>
        ...
        <div className="space-y-4">
          <SectionFieldset title="Session">
            <div className="flex items-center gap-2">
              ...
              <Select
                value={examReportSession}
                onChange={(e) => setExamReportSession(e.target.value)}
                className="w-40"
              >
                {SESSION_OPTIONS.map((s) => (
                  <option key={s} value={s}>
            ...
            {(() => {
              function MarksTable({
                ...
                const gridColumns: GridColDef[] = columns.map((c) => ({
                  ...
                  sortable: false,
                  filterable: false,
                  renderCell: (params) => params.value as ReactNode,
                }));

                const gridRows = rows.map((r, idx) => ({ id: idx + 1, ...r }));
                  ...
                  body: (
                    <MarksTable
                      columns={[
                        ...
                        { key: "obtained", label: "Obtained", align: "right" },
                      ...
                      rows={[
                        ...
                        grade: (
                          <Chip size="small" color="success" label={r.grade} />
                        ),
                      }))}
                      footer={
                        <div className="pt-3 border-t border-gray-200">
                          <strong>Total:</strong> 425 / 500 — Percentage:{" "}
                          <strong>85%</strong>
                        </div>
                      }
                    />
                ...
                },
                {
                  ...
                  body: (
                    <MarksTable
                      ...
                      rows={[
                        { subject: "Mathematics", marks: "18/25", grade: "B+" },
                        { subject: "Science", marks: "22/25", grade: "A" },
                        { subject: "English", marks: "20/25", grade: "A" },
                      ].map((r) => ({
                        subject: r.subject,
                        marks: <span className="font-medium">{r.marks}</span>,
                        grade: (
                          <Chip size="small" color="success" label={r.grade} />
                        ),
                      }))}
              ...
              const filteredExams = examList.filter(
                (e) => e.session === examReportSession,
              );
              return (
                <>
                  {filteredExams.length === 0 ? (
                    <p className="text-sm text-gray-500 py-4 text-center">
                      No exam reports for this session.
                    </p>
                  ) : (
                    filteredExams.map((exam) => {
                      ...
                      return (
                        ...
                        >
                          <button
                            type="button"
                            className="w-full flex items-center justify-between px-4 py-3 text-left text-sm font-semibold text-gray-900 bg-gray-50 hover:bg-gray-100 transition-colors"
                            onClick={() =>
                              setExamExpandedId(isOpen ? null : exam.id)
                            }
                          >
      ...
      {tab === "addNotes" && (
        <div className="space-y-4">
          <SectionFieldset title="Add New Note">
            <textarea
              className="w-full min-h-[100px] rounded-xl border border-gray-200 bg-white px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-indigo-600"
          ...
          <SectionFieldset title="Previous Notes">
            <ul className="space-y-3 text-sm">
              {notesList.map((note) => (
                <li
                  key={note.id}
                  className="rounded-xl border border-gray-200 bg-gray-50/50 p-3 flex items-start justify-between gap-2"
                >
                  ...
                  <button
                    ...
                    className="shrink-0 text-red-600 hover:text-red-700 font-medium text-xs p-1 rounded hover:bg-red-50"
                    title="Delete note"
                  >
                    Delete
                  </button>
